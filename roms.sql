...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `registerDuplicate_BEFORE_UPDATE_1` BEFORE UPDATE ON `register` FOR EACH ROW BEGIN
	IF EXISTS (
		SELECT *
        FROM register
        WHERE DateUse = NEW.`DateUse` AND ((register.`Start` <= NEW.`Start` AND register.`End` >= NEW.`Start`)
        OR (register.`Start` <= NEW.`End` AND register.`End` >= NEW.`End`))
    )
    THEN
		SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Duplicate register';
...
BEGIN
    ...
    WHERE RoomID = id AND ((DateUse = CURDATE() AND (Start + 5 >= HOUR(CURRENT_TIME()) OR End + 5 >= HOUR(CURRENT_TIME()))) OR DateUse > CURDATE())
...
BEGIN
	...
	WHERE UserID = id AND ((DateUse = CURDATE() AND Start + 5 >= HOUR(CURRENT_TIME())) OR (DateUse > CURDATE()))
	ORDER BY DateUse, Start;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2025-05-19 19:24:58
